...
void _help()
{
    ...
    printf("Example:/project/tools1/bin/procctl 20 /project/tools1/bin/tcpputfiles /log/idc/tcpputfiles_surfdata.log \"<ip>124.222.210.41</ip><port>5005</port><ptype>1</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><clientpathbak>/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\"\n");
    printf("        /project/tools1/bin/procctl 20 /project/tools1/bin/tcpputfiles /log/idc/tcpputfiles_surfdata.log \"<ip>124.222.210.41</ip><port>5005</port><ptype>2</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><clientpathbak>/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\"\n\n\n");
    ...
    printf("matchname       待上传文件匹配的规则，如\"*.TXT,*.XML\"。\n");
...
bool _xmltoarg(char *strxmlbuffer)
{
    ...
    GetXMLBuffer(strxmlbuffer, "ptype", &starg.ptype);   // 文件上传成功后的处理方式：1-删除文件；2-移动到备份目录。
    if(starg.ptype != 1 && starg.ptype != 2) { logfile.Write("ptype not in (1,2).\n"); return false;}

    GetXMLBuffer(strxmlbuffer, "clientpath", starg.clientpath);   // 本地文件存放的根目录。
    if(strlen(starg.clientpath) == 0)
    { logfile.Write("clientpath is null.\n"); return false;}
    
    GetXMLBuffer(strxmlbuffer, "clientpathbak", starg.clientpathbak, 300);   // 文件成功上传后，本地文件备份的根目录，当ptype=2时有效。
    if(strlen(starg.clientpathbak) == 0)
    { logfile.Write("clientpathbak is null.\n"); return false;}

    GetXMLBuffer(strxmlbuffer, "andchild", &starg.andchild);   // 是否上传clientpath目录下各级子目录的文件，true-是;false-否，缺省为false。
...
bool AckMessage(const char *strrecvbuffer)
{
    ...
    GetXMLBuffer(strrecvbuffer, "filename", filename, 301);
    ...
    if(starg.ptype == 2)
    {
        ...
        if(RENAME(filename, bakfilename) == false) { logfile.Write("RENAME(%s,%s) failed.\n", filename, bakfilename); return false; }
    }
