...
int main(int argc, char* argv[])
{
    ...
    // 如果starg.remotecols或starg.localcols为空，就用starg.localtname表的全部列表填充。
...
void _help()
{
    ...
    printf("        /project/tools1/bin/procctl 10 /project/tools1/bin/syncupdate /log/idc/syncupdate_ZHOBTMIND2.log \"<localconnstr>127.0.0.1,root,root,mysql,3306</localconnstr><charset>utf8</charset><fedtname>LK_ZHOBTMIND1</fedtname><localtname>T_ZHOBTMIND2</localtname><remotecols>obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</remotecols><localcols>stid,ddatetime,t,p,u,wd,wf,r,vis,upttime,recid</localcols><where>where ddatetime>timestampadd(minute,-120,now())</where><synctype>2</synctype><remoteconnstr>124.222.210.41,root,root,mysql,3306</remoteconnstr><remotetname>T_ZHOBTMIND1</remotetname><remotekeycol>keyid</remotekeycol><localkeycol>recid</localkeycol><maxcount>300</maxcount><timeout>50</timeout><pname>syncupdate_ZHOBTMIND2</pname>\"\n\n");
    ...
    printf("where           同步数据的条件，为空则表示同步全部的记录，填充在delete本地表和select Federated表之后，注意：1）where中的字段必须同时在本地表和Federated表中；2）不要用系统时间作为条件。\n");
    printf("synctype        同步方式：1-不分批同步；2-分批同步。\n");
    printf("remoteconnstr   远程数据库的连接参数，格式与localconstr相同，当synctype==2时有效。\n");
...
bool _xmltoarg(char *strxmlbuffer)
{
    ...
    if(starg.synctype == 2)
    {
        // 远程数据库的连接参数，格式与localconstr相同，当synctype==2时有效。
...
bool _syncupdate()
{
    ...
    if(starg.synctype == 1)
    {
        ...
        // 再把starg.fetname表中满足where条件的记录插入到starg.localtname表中。
        ...
        if(stmtins.execute() != 0)
        {
            ...
            connloc.rollback();     // 如果这里失败了，一定要回滚事务。connection类的析构函数会回滚。
            return false;
        }
