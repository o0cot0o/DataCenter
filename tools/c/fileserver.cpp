...
int main(int argc,char *argv[])
{
    if (argc!=3)
    {
        ...
        printf("Example:./project/tools1/bin/fileserver 5005 /log/idc/fileserver.log\n");
...
bool ClientLogin()
{
    ...
    if(TcpServer.Write(strsendbuffer, 20) == false)
    {
...
bool _xmltoarg(char *strxmlbuffer)
{
    ...
    GetXMLBuffer(strxmlbuffer, "srvpathbak", starg.srvpathbak);     // 服务端文件存放的根目录。

    GetXMLBuffer(strxmlbuffer, "timetvl", &starg.timetvl);          // 扫描本地目录文件的时间间隔，单位：秒，取值在1-30之间。
    if(starg.timetvl > 30) starg.timetvl = 30;                      // timeval没有必要超过30秒
...
void RecvFilesMain()
{
    ...
    while(true)
    {
        ...
        // 接受客户端的报文
        // 第二个参数的取值必须大于starg.timeval, 小于starg.timeout
        if(TcpServer.Read(strrecvbuffer, starg.timeout+10) == false)
        {
            logfile.Write("TcpServer.Read() failed.\n"); return;
        }
        // logfile.Write("strrecvbuffer=%s\n", strrecvbuffer);
