...
{
    m_allcount = m_pkcount = m_maxcollen = 0;
    m_vallcols.clear();
    m_vpkcols.clear();
...
bool CTABCOLS::allcols(connection *conn, char *tablename)
{
    ...
    while(true)
    {
        ...
        // 如果业务有需求，可以修改上面的代码，增加对更多数据类型的支持。
...
bool CTABCOLS::pkcols(connection *conn, char *tablename)
{
    ...
    stmt.prepare("select lower(column_name), position from USER_CONS_COLUMNS \
            where table_name=upper(:1) and constraint_name=(select constraint_name from USER_CONSTRAINTS \
